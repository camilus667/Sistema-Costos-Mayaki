...
export async function generarExcelColegioBuffer(
  grupo: string,
  tipo: 'ambos' | 'precios' | 'stock' = 'ambos'
): Promise<Buffer> {
  ...
  matriz.filas.forEach((fila) => {
    const rowValues: (string | number)[] = [fila.cod, fila.nombreLimpio];
  ...
  if (tipo === 'stock') {
    const totalesTallas: Record<string, number> = {};
    let granTotal = 0;
    ...
    const totalRowValues: (string | number)[] = ['', 'TOTAL INVENTARIO'];
...
export async function generarExcelGlobalBuffer(): Promise<Buffer> {
  const workbook = new ExcelJS.Workbook();
...
export async function generarXlsxPos38Buffer(): Promise<Buffer> {
  ...
  const filasSalida: any[][] = [cabeceraOriginal];
