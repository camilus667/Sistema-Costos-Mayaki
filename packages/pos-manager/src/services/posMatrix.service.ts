...
const ABREVIATURAS_GRUPO: Record<string, string> = {
  'Cambridge': 'CC',
...
async function resolverAbreviaturaColegio(grupo: string): Promise<string> {
  try {
...
export async function obtenerGrupos(): Promise<PosGrupoSummary[]> {
  const db = await getDb();
  const todos = db.select().from(posProductos).all();

  const conteos: Record<string, { filas: number; productos: Set<string> }> = {};

  todos.forEach((p) => {
    const g = p.grupoMatriz;
    if (!conteos[g]) conteos[g] = { filas: 0, productos: new Set() };
    conteos[g].filas++;
...
export async function obtenerMatrizGrupo(grupo: string): Promise<MatrizRespuesta> {
  ...
  const mapaFilas: Map<string, { nombreLimpio: string; orden: number; items: typeof productosGrupo }> = new Map();
  ...
  const filas: MatrizFila[] = filasOrdenadas.map((f, idx) => {
...
export async function obtenerListaReferencia(): Promise<ReferenciaItem[]> {
  const db = await getDb();
...
export async function actualizarProductoPos(
  id: string,
  cambios: { precioPos?: number | null; cantInvGeneral?: number | null }
) {
  const db = await getDb();
  ...
  let datosOrig: Record<string, any> = {};
  try {
...
export async function actualizarOrdenProductos(grupo: string, ordenPosIds: string[]) {
  const db = await getDb();
